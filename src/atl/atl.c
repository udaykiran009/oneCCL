...
static void atl_ini_dir(int *argc, char ***argv, size_t *proc_idx, size_t *proc_count,
                        ...
                        const char *dir, char *transport_name)
{
    int n = 0;
    char *lib;
    void *dlhandle;
    struct dirent **liblist = NULL;
    atl_status_t* (*inif)(atl_transport_t *);
	...
	if (inif == NULL) {
            ...
            if (strncmp(transport.name, transport_name,
                        MIN(transport_name_len, strlen(transport.name))))
                continue;
