...
public:
    ...
    recv_reduce_entry(mlsl_sched* sched,
                      ...
                      size_t src,
                      void* comm_buf) :
        sched_entry(sched), inout_buf(inout_buf),
        in_cnt(in_cnt), out_cnt(out_cnt), dtype(dtype),
        op(reduction_op), src(src), comm_buf(comm_buf)
    {
...
protected:
    char* dump_detail(char* dump_buf) const
    {
        auto bytes_written = sprintf(dump_buf, "dt %s, inout_buf %p, in_cnt %zu, out_cnt %p, op %s, red_fn %p, "
                                     ...
                                     src, comm_buf, sched->coll_param.comm, &req);
