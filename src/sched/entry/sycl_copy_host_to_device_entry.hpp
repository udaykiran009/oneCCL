...
public:
    ...
    void start() override
    {
        ...
        auto out_buf_acc = static_cast<ccl_sycl_buffer_t*>(out_buf.get_ptr(bytes))->get_access<cl::sycl::access::mode::discard_write>();
        CCL_ASSERT(cnt <= out_buf_acc.get_count());
        auto comp_status = ccl_comp_copy(in_buf.get_ptr(bytes), out_buf_acc.get_pointer(), cnt, dtype);
