...
public:
        ...
        op(reduction_op), src(src), comm_buf(comm_buf), op_id(op_id), fn(sched->coll_attr.reduction_fn)
    {
        ...
        {
            this->comm_buf = ICCL_MALLOC(in_cnt * iccl_datatype_get_size(dtype), "recv_reduce.comm_buf");
    ...
    void update_derived()
    {
        ...
        if (req_status)
        {
            ...
            iccl_status_t comp_status = iccl_comp_reduce(comm_buf, in_cnt, inout_buf, out_cnt,
                                                         dtype, op, fn);
