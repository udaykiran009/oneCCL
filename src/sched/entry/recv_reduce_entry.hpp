...
public:
        ...
        op(reduction_op), src(src), comm_buf(comm_buf), op_id(op_id), fn(sched->coll_attr.reduction_fn)
    {
        ...
        {
            size_t comm_buf_size = in_cnt * iccl_datatype_get_size(dtype);
            this->comm_buf.set(ICCL_MALLOC(comm_buf_size, "recv_reduce.comm_buf"), comm_buf_size);
            own_comm_buff = true;
        }
    }
    ...
    void update_derived()
    {
        ...
        if (req_status)
        {
            ...
            size_t bytes = in_cnt * iccl_datatype_get_size(dtype);
            iccl_status_t comp_status = iccl_comp_reduce(comm_buf.get_ptr(bytes), in_cnt,
