...
details::cross_device_rating
    property_p2p_rating_calculator(const native::ccl_device& lhs,
...
CCL_API void* ccl_device::device_alloc_shared_memory(size_t bytes_count, size_t alignment,
                                                     const ze_host_mem_alloc_desc_t& host_desc,
                                                      const ze_device_mem_alloc_desc_t& mem_descr)
{
    void* out_ptr = nullptr;
    ze_result_t ret = zeDriverAllocSharedMem(get_owner().lock()->handle, &mem_descr, &host_desc,
                                             bytes_count, alignment, handle, &out_ptr);

    if(ret != ZE_RESULT_SUCCESS )
    {
        throw std::runtime_error(std::string("cannot allocate shared memory, error: ") + std::to_string(ret));
    }
...
CCL_API ccl_device::device_module_ptr
        ccl_device::create_module(const ze_module_desc_t &descr, size_t hash)
{
    ...
    {
        throw std::runtime_error(std::string(__FUNCTION__) +" - failed: hash conflict for: " +
                                 std::to_string(hash));
    }
    return module_ptr;
}
