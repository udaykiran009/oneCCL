...
struct sparse_allreduce_usm_visitor
{
    ...
    {
        return static_cast<const self_t*>(const_cast<sparse_allreduce_usm_visitor*>(this)->get_self());
    }

    template<class ...Args>
    bool visit(coll_request_t&req, ccl::datatype index_dtype, ccl::datatype value_dtype,
                        ...
                        Args&&...args)
    {
        ...
        LOG_TRACE("comm: ", get_self()->to_string(), " - starting to find visitor for datatype: ", ccl::to_string(value_dtype), " , handle: ", utils::enum_to_underlying(value_dtype));
        switch(value_dtype) //TODO -S- value only
        {
            case ccl::datatype::int8:
            {
                using type = char;
                req = get_self()->template sparse_allreduce_impl<type>(static_cast<const type*>(send_ind_buf),
                                                   send_ind_count,
            ...
            case ccl::datatype::uint8:
            {
                throw ccl::ccl_error(std::string(__PRETTY_FUNCTION__) +
                            " - USM convertation of value_dtype: " + ccl::to_string(value_dtype) + " is not supported for such configuration");
                break;
            }
            ...
            default:
            {
                LOG_DEBUG("comm: ", get_self()->to_string(), " - no found visitor for datatype: ", ccl::to_string(value_dtype), " , handle: ", utils::enum_to_underlying(value_dtype),
                         ", use RAW types");
                break;
            }
        }
