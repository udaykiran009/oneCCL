...
void device_community_container<class_id>::bind_device_by_id(
    ...
    storage->template bind_device_by_id<group_id>(device_id, registered_addr, out_device_binder,
                                                  preferred_rank);
}
...
void device_community_container<ccl::device_topology_type::ring>::bind_device_by_id(
    ...
    for (auto it = closed_rings.begin(); it != closed_rings.end(); ++it) {
        (*it)->template bind_device_by_id<group_id>(device_id, registered_addr, out_device_binder, preferred_rank);
    }
