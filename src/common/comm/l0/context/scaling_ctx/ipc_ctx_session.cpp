...
void session::start(net::ipc_client* client, const std::string& addr) {
    ...
    *reinterpret_cast<size_t*>(payload.data() + sizeof(source_device_rank)) = send_tag;
    source_ipc_raw_data = tx_connection->send_ipc_memory_ext(source_ipc_memory_storage, payload.data(), payload.size());

    //TODO RING only: peer-to-peer
    data_to_recover.raw_data.resize(source_ipc_raw_data.size());
    LOG_DEBUG("Rank: ", source_device_rank, ", prepared IPC handles exhange bytes for receive:", data_to_recover.raw_data.size());
}
                      ...
                      const net::ipc_rx_connection* incoming_connection)
{
    ...
    LOG_DEBUG("session: ", reinterpret_cast<void*>(this), ", recovered ipc storage size: ", existing_recovered_ipc_size);
    ...
    received_tag = *reinterpret_cast<size_t*>(data_to_recover.raw_data.data() + sizeof(source_device_rank));
    (void)received_tag;
    ...
    for (auto &recv_ip_handle : handles)
    {
        ...
        LOG_DEBUG("Found IPC owner comm device: ", indexed_ipc_dst_devices->to_string(),
        ...
        try {
            ...
            data_to_recover.ipc_memory_storage[indexed_ipc_dst_devices].push_back(std::move(restored));
...
std::string session::to_string() const
{
    std::stringstream ss;
    ss << "tag: " << send_tag
       << ", src_dev_rank: " << source_device_rank
