...
public:
    ...
    virtual void visit(const ccl_ipc_gpu_comm* source, native::supported_device_modules<ipc_dst_device_coll_module>& ipc_modules) = 0;
protected:
...
struct typed_ipc_session : public session {
    typed_ipc_session(origin_ipc_memory_container&& ipc_src_memory_handles, size_t source_ipc_device_rank) :
    ...
    void visit(const ccl_ipc_gpu_comm* source,
        ...
        auto& module_ptr = std::get<::utils::enum_to_underlying(class_id)>(
            std::get<::utils::enum_to_underlying(ccl::group_split_type::cluster)>(std::get<coll_type>(ipc_modules)));
        assert(module_ptr);
        ...
        // bind data
        const recovered_handles_storage::restored_ipc_memory_container& ipc_handles = data_it->second;
...
struct session_table
{
    ...
    std::shared_ptr<session> create_session(const session_key_t& key,
        ...
        using specific_session = typed_ipc_session<ipc_invoke_params_type::get_coll_type(),
                                                   ...
                                                   class_id>;
        auto sess = std::make_shared<specific_session>(std::move(params.handles), source_device_rank);
...
private:
     void start_session(std::shared_ptr<session> sess, net::ipc_client* client,
                        const std::string& peer_addr);
};
