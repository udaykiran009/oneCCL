...
ssize_t connection::send_data(const uint8_t* send_data_ptr, size_t size, int flags) const {
    ...
    if (ret == -1 && (errno != EAGAIN || errno != EWOULDBLOCK)) {
        throw std::runtime_error(std::string("Cannot send data to socket: ") + std::to_string(socket_fd) +
                                 strerror(errno));
    }
...
ssize_t connection::recv_data(uint8_t* recv_data_ptr, size_t size, int flags) const {
    ...
    if (ret == -1 && (errno != EAGAIN || errno != EWOULDBLOCK)) {
        throw std::runtime_error(std::string("Cannot recv data from socket: ") + std::to_string(socket_fd) +
                                 strerror(errno));
    }
...
ssize_t connection::send_msg_with_pid_data(const std::vector<uint8_t>& data,
                                           const std::vector<size_t>& optional_pid_data_offets, int flag) const {
    ...
    {
        LOG_ERROR("ancillary_data_limit_bytes is to less: ", connection::ancillary_data_limit_bytes(), "bytes, than required: ",
                  optional_pid_data_offets.size() * sizeof(fd_t), ". Recompile with large limits is required");
        abort();
    }
    ...
    msg.msg_iov = &io;
    msg.msg_iovlen = 1;
    msg.msg_controllen = CMSG_SPACE(sizeof(fd_t) * optional_pid_data_offets.size()); //sizeof(u.buf);
    ...
    for(auto fd_offset_bytes_it = optional_pid_data_offets.begin();
        fd_offset_bytes_it!= optional_pid_data_offets.end(); ++fd_offset_bytes_it)
    {
        if (*fd_offset_bytes_it >= data.size()) {
            throw std::runtime_error(std::string(__FUNCTION__) + "Unexpected value in optional_pid_data_offets, data size: " +
                                     std::to_string(data.size()) + ", offset requested: " +
                                     std::to_string(*fd_offset_bytes_it));
        }
...
ssize_t connection::recv_msg_with_pid_data(std::vector<uint8_t>& out_data_resized, std::vector<fd_t>& out_pids_resized, int flags) const {

    LOG_DEBUG("Prepared data size bytes: ", out_data_resized.size(),
              ", pid count: ", out_pids_resized.size(), ", socket: ", socket_fd);
    ...
    if (bytes_got == -1) {
        throw std::runtime_error(std::string(__FUNCTION__) + " - cannot receive data, error: " + strerror(errno));
    }
    ...
    for (; cmsg != nullptr; cmsg = CMSG_NXTHDR(&msg_header, cmsg) ) {
        ...
        if ( cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS ) {
            ...
            if(cmsg->cmsg_len < expected_len) {
                throw std::runtime_error(std::string(__FUNCTION__) + " - got unexpected anciliary msg size from socket: " + std::to_string(socket_fd) +
                                         ", got: " + std::to_string(cmsg->cmsg_len) + ", but expected: " +
            ...
            {
                it = *fd_ptr;
                LOG_DEBUG("got fd: ", *fd_ptr, ", by number: ", received_fd_num, ", expected count: ", out_pids_resized.size());
    ...
    if (received_fd_num != out_pids_resized.size()) {
            throw std::runtime_error(std::string(__FUNCTION__) + " - unexpected FD from socket: " + std::to_string(socket_fd) +
                                     ", received count: " + std::to_string(received_fd_num) + ", but expected: " +
                                     std::to_string(out_pids_resized.size()));
    }
    return bytes_got;
}
}
