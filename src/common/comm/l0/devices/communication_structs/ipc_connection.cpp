...
ipc_rx_connection::ipc_rx_connection(int socket) :
    connection(socket) {
...
std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>>
            ipc_rx_connection::receive_ipc_memory(std::vector<uint8_t>& out_data_resized, size_t &out_received_rank) const {
...
std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>>
            ipc_rx_connection::receive_ipc_memory_ext(std::vector<uint8_t>& out_data_resized, size_t out_data_offset_bytes) const {
    ...
    std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>> ret;
    LOG_DEBUG("Expected receive bytes: ", out_data_resized.size(), ", handles count: ", handles_count, ", bytes in rest: ", bytes_rest);
    try {
        if (bytes_rest != 0) {
            throw std::runtime_error(std::string("Unexpected bytes to receive: ") + std::to_string(out_data_resized.size()) +
                                     ", handles count: " + std::to_string(handles_count) + ", bytes in rest should be zero, got: " +
                                     std::to_string(bytes_rest));
        }
        ...
        for(auto &handle : ret) {
            ...
            LOG_DEBUG("Override FD: ", *pid_ptr, ", with new FD: ", new_pid, ", for IPC handle num: ", num_handles);
            *pid_ptr = new_pid;
            num_handles++;
        }
    }
...
std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>>
    ipc_rx_connection::receive_raw_ipc_memory(std::vector<uint8_t>& out_data_resized, std::vector<connection::fd_t>& out_pids_resized, size_t &received_rank) const {
...
std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>>
    ...
    LOG_DEBUG("Try to receive ip memory for expected bytes count: ",
              out_data_resized.size(), ", fd count: ", out_pids_resized.size(),
              ", with offset", out_data_offset_bytes);
    if (out_data_resized.size() < out_data_offset_bytes) {
        LOG_ERROR("not enough bytes in out_data_resized: ", out_data_resized.size(), ", for given offset: ", out_data_offset_bytes);
        abort();
    }

    ssize_t read_bytes = 0;
    try {
        read_bytes = recv_msg_with_pid_data(out_data_resized, out_pids_resized, MSG_CMSG_CLOEXEC | MSG_WAITALL);
        LOG_DEBUG("Read bytes count: ", read_bytes);

        if (static_cast<size_t>(read_bytes) < out_data_resized.size()) {
            throw std::runtime_error(std::string("Too many bytes received: ") + std::to_string(read_bytes));
        }
    ...
    while (num_handles < expected_handles and recv_data_size > 0) {
        ...
        LOG_DEBUG("Start restore handle num: ", num_handles, ", expected count: " , expected_handles);
        try {
            // deserialize handle
            auto recv_ip_handle =
                native::ccl_device::device_ipc_memory_handle::deserialize<
        ...
        {
            LOG_ERROR("Cannot deserialize IPC handle by index: ", num_handles, ", error:\n",
                      ex.what());
            throw;
        }
        num_handles++;
...
ipc_tx_connection::ipc_tx_connection(const std::string& addr) :
    connection(-1) {
    ...
    try
    {
        ...
        if (socket_fd == -1) {
            throw std::runtime_error(std::string("Cannot create client socket, error: ") + strerror(errno));
        }
        ...
        // make connect
        int ret = -1;
        while((ret = connect (socket_fd, (const struct sockaddr *) &peer_addr,
                          sizeof(struct sockaddr_un))) == -1)
            if (errno != EINTR && errno != EINPROGRESS) {
                throw std::runtime_error(std::string("Cannot connect socket: ") + std::to_string(socket_fd) + " to peer: " + addr + ", error: " + strerror(errno));
            }
    }
    catch(const std::exception& ex) {
        LOG_ERROR(ex.what());
...
std::vector<uint8_t> ipc_tx_connection::send_ipc_memory(const std::vector<native::ccl_device::device_ipc_memory_handle>& handles, size_t send_rank) const {
...
std::vector<uint8_t> ipc_tx_connection::send_ipc_memory_ext(
    ...
    size_t bytes_to_send = handle_size * handles.size() + out_raw_data_initial_offset_bytes; //ipc_data + out_raw_data_initial_offset_bytes
    ...
    for (const auto& ipc_handle : handles) {
        ...
        pids_offset_bytes.push_back(serialize_offset - sizeof(native::ccl_device::device_ipc_memory_handle::handle_t));

        LOG_DEBUG("Serialized bytes: ", serialize_offset, ", with pid offset by: ", pids_offset_bytes.back());
    }

    memcpy(reinterpret_cast<uint8_t*>(out_raw_data.data()), payload, out_raw_data_initial_offset_bytes);
    ...
    LOG_DEBUG("Handles serialized count: ", handles.size(), ", data bytes: ", serialize_offset,
              ", sent bytes: ", send_bytes);
    return out_raw_data;
}
}
