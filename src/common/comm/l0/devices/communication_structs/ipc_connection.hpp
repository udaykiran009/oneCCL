...
public:
    ...
    std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>>
            receive_ipc_memory(std::vector<uint8_t>& out_data_resized, size_t &out_received_rank) const;

    // For properly call resize data with *_resized before for expected sizes
    std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>>
            receive_ipc_memory_ext(std::vector<uint8_t>& out_data_resized, size_t out_data_offset_bytes) const;

    // For properly call resize data with *_resized before for expected sizes
    std::vector<std::shared_ptr<native::ccl_device::device_ipc_memory_handle>>
            receive_raw_ipc_memory(std::vector<uint8_t>& out_data_resized, std::vector<connection::fd_t>& out_pids_resized, size_t &out_rank) const;
...
public:
    ...
    std::vector<uint8_t> send_ipc_memory(const std::vector<native::ccl_device::device_ipc_memory_handle>& handles, size_t send_rank) const;
    std::vector<uint8_t> send_ipc_memory_ext(const std::vector<native::ccl_device::device_ipc_memory_handle>& handles,
                                             const uint8_t* payload = nullptr, size_t payload_size = 0) const;
private:
