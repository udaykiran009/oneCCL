...
template<ccl::device_group_split_type group_id,
         ccl::device_topology_type class_id>
template<class device_t>
void rank_getter<group_id, class_id>::operator() (const native::indexed_device_container<device_t>& container)
{
    if(find)
    {
        return;
...
template<ccl::device_group_split_type group_id,
         ccl::device_topology_type class_id>
template<class device_t>
void rank_getter<group_id, class_id>::operator() (const native::plain_device_container<device_t>& container)
{
    if(find)
    {
        return;
...
size_t rank_getter<group_id, class_id>::get_assigned_rank() const
{
    if (!find)
    {
        throw std::runtime_error(std::string(__FUNCTION__) +
                                 "rank_getter doesn't assign rank for device: " +
                                  ccl::to_string(device_id));
    }
    return rank;
}
}
