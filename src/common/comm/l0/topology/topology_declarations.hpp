...
{
    colored_index(color_t c, const ccl::device_index_type& i) :
        color(c), index(i)
    {}
    color_t color;
...
struct colored_indexed_data : public colored_index
{
    ...
    colored_indexed_data(color_t c, const ccl::device_index_type& i,
...
using global_plain_colored_graphs = std::vector<std::pair<process_index_t, colored_plain_graph_list>>;
...
std::ostream& operator<<(std::ostream& out, const details::colored_indexed_data<payload_type>& data)
{
    out << data.to_string();
    return out;
}
