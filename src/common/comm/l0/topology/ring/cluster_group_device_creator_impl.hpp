...
inline
cluster_group_device_creator::cluster_group_device_creator(size_t process_idx,
                                                           size_t process_nums,
...
inline size_t
    cluster_group_device_creator::default_property_p2p_rating_calculator(const ccl_device &lhs,
                                                                         const ccl_device &rhs)
{
    return details::property_p2p_rating_calculator(lhs, rhs, PROCESS_GROUP_WEIGHT);
}

inline details::adjacency_matrix
    cluster_group_device_creator::build_p2p_capability_matrix(
                                            std::ostream& out,
                                            ...
                                            details::p2p_rating_function ping)
{
    ...
    return thread_group_ring_topology::build_p2p_capability_matrix(out,
                                                                   single_node_device_indices,
                                                                   ping);
}

inline
bool cluster_group_device_creator::build_all(std::ostream& out,
                                             const ccl::context_comm_addr& comm_addr,
                                             const ccl::process_device_indices_t&
                                             ...
                                             details::p2p_rating_function ping)
{
    ...
    details::plain_graph_list my_device_graphs =
                            details::graph_list_resolver(single_node_matrix,
                                                         cur_process_per_thread_device_indices,
                                                         ping);
    ...
    for (const auto& node_conf : context.cluster_gpu_indices)
    {
        ...
        for (auto it = processes.begin();
             it != processes.end() && symm_test;
             ++it)
        {

            ccl::device_indices_t result_intersection;
            std::set_intersection(it->second.begin(), it->second.end(),
                                  node_device_intersection.begin(), node_device_intersection.end(),
    ...
    out << "\nProcs on nodes:\t";
    std::copy(processes_on_node.begin(), processes_on_node.end(),
              std::ostream_iterator<size_t>(out, ","));
    out << "\nIPCs devices on nodes:\t" ;
    std::copy(ipc_devices_on_node.begin(), ipc_devices_on_node.end(),
              std::ostream_iterator<size_t>(out, ","));
    out << std::endl;
    ...
    if (symm_test && not ipc_devices_candidates.empty())
    {
        ...
        else
        {
            ...
            std::for_each(scale_out_devices.begin(), scale_out_devices.end(),
    ...
    else
    {
        ...
        bool process_symmetric_test = (procs_on_node != 1); //nothing to scale-up for 1 process on node
        process_symmetric_test &= std::all_of(processes_on_node.begin(), processes_on_node.end(),
                                                  [procs_on_node](size_t val)
        ...
        if (process_symmetric_test)
        {
            ...
            scale_up_devices.emplace_back(scale_up_color,
                                          my_colored_graphs.begin()->begin()->index,
                                          thread_index);
            ...
            else
            {
                ...
                if (scale_out_color == scale_up_color)
                {
                    ...
                    out << "UNHANDLED CASE: scale-up & scale-out comm color is the same!\n"
                        << "Reassign one of them: plus '1', re hash and broad-cast it!" << std::endl;
                    abort();
                }
            }
        }
    ...
    return build_impl<ccl::device_topology_type::ring>
                     (out, comm_addr, cur_process_per_thread_device_indices,
                      single_node_matrix,
...
inline bool cluster_group_device_creator::build_impl(std::ostream& out,
                   const ccl::context_comm_addr& comm_addr,
                   ...
                   details::p2p_rating_function ping/* = default_property_p2p_rating_calculator*/)
{
    ...
    for (const auto& id_ring : graph_list)
    {
        ...
        for(const auto& per_thread : cur_process_per_thread_device_indices)
        {
            ...
            if (per_thread.second.find(last_in_graph_index) != per_thread.second.end())
            {
                ...
                out << "thread: " << thread_id << " wants to create numa-proxy device by idx: "
                    << last_in_graph_index << std::endl;
                if (created_cpu_context_indices.find(last_in_graph_index) != created_cpu_context_indices.end())
                {
                    out << "skip existing numa-proxy device candidate by: " << last_in_graph_index << std::endl;
                    continue;
                }

                auto proxy_virt = details::add_numa_proxy_device<ccl_virtual_gpu_comm,
                                                                 group_id(),
                                                                 class_id>(
                ...
                {
                    auto proxy_real = details::add_numa_proxy_device<ccl_gpu_comm,
                                                                     group_id(),
                                                                     class_id>(
                                                                    *non_indexed_plain_devices,
                                                                    last_in_graph_index,
    ...
    for (auto& id_ring : graph_list)
    {
             ...
             ++per_thread_it)
        {
            ...
            if (graph_list.size() ==1)
            {
                if (context.get_process_topology<class_id>(process_index, thread_id).closed_rings.empty())
                {
                    context.get_process_topology<class_id>(process_index, thread_id).set_topology( std::make_shared<device_community<class_id>>(topology_comm_addr));
                }

                out_indexed_devices = context.get_process_topology<class_id>(process_index, thread_id).get_topology(ring_index);
            }
            else
            {
                if (context.get_process_topology<class_id>(process_index, thread_id).torn_apart_rings.empty())
                {
                    context.get_process_topology<class_id>(process_index, thread_id).set_additiona_topology( std::make_shared<device_community<class_id>>(topology_comm_addr));
                }

                out_indexed_devices = context.get_process_topology<class_id>(process_index, thread_id).get_additiona_topology(ring_index);
            }
            ...
            auto rank_builder =
                    create_device_functor<details::colored_graph_ring_indexer<group_id(),
                                                                              class_id>>(id_ring,
                                                                                      thread_id,
                                                                                      process_index,
            ...
            out << "\nIndexer for graph num: " << graph_num
                << ", finished. marked_indices: " << rank_builder.get_functor().get_marked_indices_count()
    ...
    for (const auto& id_ring : graph_list)
    {
            ...
            current_thread_idx++)
        {
            ...
            if (graph_list.size() ==1)
            {
                community = context.get_process_topology<class_id>(process_index, current_thread_idx).get_topology(ring_index);
            }
            else
            {

                community = context.get_process_topology<class_id>(process_index, current_thread_idx).get_additiona_topology(ring_index);
            }
            ...
            const auto& curr_real =
                    details::get_device_with_min_rank<ccl_gpu_comm,
                                                      group_id(),
                                                      class_id>(indexed_devices_for_current_thread,
                                                                id_ring);
            const auto& curr_virt =
                    details::get_device_with_min_rank<ccl_virtual_gpu_comm,
                                                      group_id(),
                                                      class_id>(indexed_devices_for_current_thread,
                                                                id_ring);
            const auto& curr_scale_real =
                    details::get_device_with_min_rank<ccl_numa_proxy<ccl_gpu_comm>,
                                                      group_id(),
                                                      class_id>(indexed_devices_for_current_thread,
                                                                id_ring);
            ...
            for(size_t next_thread_id = 0; next_thread_id < ctx_per_thread_data.size(); next_thread_id++)
            {
                ...
                if (graph_list.size() ==1)
                {
                    community = context.get_process_topology<class_id>(process_index, next_thread_id).get_topology(ring_index);
                }
                else
                {

                    community = context.get_process_topology<class_id>(process_index, next_thread_id).get_additiona_topology(ring_index);
                }
                auto& next_thread_ring_topology = community->get_device_storage();

                const auto& real =
                        details::get_device_with_max_rank<ccl_gpu_comm,
                                                          group_id(),
                                                          class_id>(next_thread_ring_topology,
                                                                    id_ring);
                const auto& virt =
                        details::get_device_with_max_rank<ccl_virtual_gpu_comm,
                                                          group_id(),
                                                          class_id>(next_thread_ring_topology,
                                                                    id_ring);
                    ...
                    auto locker =
                            details::add_concurrent_locker_device<ccl_gpu_comm,
                                                                  ...
                                                                  class_id>(next_rank,
                                                                            ...
                                                                            real,
                                                                            devices_factory,indexed_devices_for_current_thread);
        ...
        switch (syntetic_device_type_index)
        {
            ...
            case 1: //scale-up device
            {
                for (const auto& idx : colored_devices)
                {
                    ...
                    if (graph_list.size() ==1)
                    {
                        community = context.get_process_topology<class_id>(process_index, thread_id).get_topology(ring_index);
                    }
                    else
                    {

                        community = context.get_process_topology<class_id>(process_index, thread_id).get_additiona_topology(ring_index);
                    }
                    ...
                    else
                    {
                        ...
                        std::ostringstream ss;
                        ss << out.rdbuf();
                        throw std::runtime_error(std::string("Unsupported device type in topology creation. Log:\n") +
                                         ss.str());
                    }
                }
                syntetic_device_type_index ++;
            ...
            case 2: //scale-out device
            {
                for (const auto& idx : colored_devices)
                {
                    ...
                    size_t inserted_device_type_index =
                                details::inject_scaleout_device<group_id(),
                                                                class_id,
                                                               process_group_context,
