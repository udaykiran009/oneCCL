...
template<class device_t, ccl::device_group_split_type group_id, ccl::device_topology_type class_id,
         class context>
device_t_ptr<ccl_numa_proxy<device_t>>
        add_numa_proxy_device(specific_plain_device_storage &plain_storage,
                              ...
                              device_storage& device_factory)
{
    ...
    for (auto it = container.begin(); it != container.end(); ++it)
    {
        ...
        if ((*it)->get_device().get_device_path() == index)
        {
            ...
            ret = device_factory.create_gpu_device<ccl_numa_proxy<device_t>>(device->get_device(),
                                                                             container.size(),
                                                                             *device);
                                 ...
                                 color_t exclude_color = marked_color)
{
    ...
    do
    {
        ...
        auto candidate_it = ipc_indices.find(actual_ipc_process_index);
        if (candidate_it == ipc_indices.end()
            or (candidate_it->second.find(graph_rit->index) == candidate_it->second.end()))
        {
            break;
        }
...
struct graph_ring_indexer_ext : public graph_ring_indexer<group_id, class_id>
{
    ...
    graph_ring_indexer_ext(std::vector<marked_idx>& id_ring_vector,
        ...
        graph_ring_indexer<group_id, class_id>(id_ring_vector, thread_id_storage, thread_id, device_topology),
...
template<class device_t, ccl::device_group_split_type group_id, ccl::device_topology_type class_id,
         class context,
         class = typename std::enable_if<not std::is_same<device_t,
                                         ccl_thread_comm<typename device_t::impl_t>>::value>::type>
device_t_ptr<ccl_gpu_scaleup_proxy<device_t>>
        add_scaleup_device(specific_indexed_device_storage &storage,
                           const ccl::device_index_type& index,
                           context& context_to_register,
                           device_storage& device_factory)
{
        ...
        if (it->second->get_device().get_device_path() == index)
        {
            ...
            if (!inserted.second)
            {
                throw std::runtime_error(std::string(__PRETTY_FUNCTION__) +
                                         " - cannot promoted device wrapper: " +
                                         device->to_string() + " by index: " +
                                         std::to_string(index) +
...
template<class device_t, ccl::device_group_split_type group_id, ccl::device_topology_type class_id,
         class context,
         typename std::enable_if<std::is_same<device_t,
                                       ccl_thread_comm<typename device_t::impl_t>>::value, int>::type = 0>
device_t_ptr<ccl_gpu_scaleup_proxy<device_t>>
        add_scaleup_device(specific_indexed_device_storage &storage,
                           const ccl::device_index_type& index,
                           context& context_to_register,
                           device_storage& device_factory)
{
    ...
    for (auto it = container.begin(); it != container.end(); ++it)
    {
        ...
        if (it->second->get_device().get_device_path() == index)
        {
            ...
            ret = device_factory.create_gpu_device<ccl_gpu_scaleup_proxy<impl_device_t>>(device->get_device(),
                                                                                    container.size(),
                                                                                    core_dev);
            ret->template assign<group_id, class_id>(context_to_register,
                                           context_to_register.get_scaleup_ctx());
            auto inserted =
                std::get<ccl_gpu_scaleup_proxy<impl_device_t>::type_idx()>(storage).emplace(index, ret);
            if (!inserted.second)
            {
                throw std::runtime_error(std::string(__PRETTY_FUNCTION__) +
                                         " - cannot promoted device wrapper: " +
...
template<ccl::device_group_split_type group_id, ccl::device_topology_type class_id,
         class context, class ...device_candidate_t>
                             ...
                             device_storage& device_factory)
{
    ...
    std::array<bool, sizeof...(device_candidate_t)> expander {
        (created = (created == false ?
                    (bool)add_scaleup_device<device_candidate_t,
                                       group_id,
                                       class_id,
                                       context>(storage,
                                                index,
    ...
    return inserted_it != expander.end() ?
                            std::distance(expander.begin(), inserted_it) :
...
template<class device_t, ccl::device_group_split_type group_id, ccl::device_topology_type class_id,
         ...
         class = typename std::enable_if<not std::is_same<device_t,
                                         ccl_thread_comm<typename device_t::impl_t>>::value>::type>
device_t_ptr<ccl_scaleout_proxy<device_t>>
        add_scaleout_device(specific_indexed_device_storage &storage,
                           const ccl::device_index_type& index,
                           context& context_to_register,
                           device_storage& device_factory)
{
    ...
    for (auto it = container.begin(); it != container.end(); ++it)
    {
        ...
        if (it->second->get_device().get_device_path() == index)
        {
            ...
            ret = device_factory.create_gpu_device<ccl_scaleout_proxy<device_t>>(device->get_device(),
                                                                                    container.size(),
                                                                                    *device);
            ret->template assign<group_id, class_id>(context_to_register,
                           ...
                           device_storage& device_factory)
{
    ...
    for (auto it = container.begin(); it != container.end(); ++it)
    {
        ...
        if (it->second->get_device().get_device_path() == index)
        {
            ...
            ret = device_factory.create_gpu_device<ccl_scaleout_proxy<impl_device_t>>(device->get_device(),
                                                                                    container.size(),
                                                                                    *device);
            ret->template assign<group_id, class_id>(context_to_register,
                                           context_to_register.get_scaleout_ctx());
            auto inserted =
                std::get<ccl_scaleout_proxy<impl_device_t>::type_idx()>(storage).emplace(index, ret);
            if (!inserted.second)
            {
                throw std::runtime_error(std::string(__PRETTY_FUNCTION__) +
                                         " - cannot promoted device wrapper: " +
                             ...
                             device_storage& device_factory)
{
    ...
    std::array<bool, sizeof...(device_candidate_t)> expander {
        (created = (created == false ?
                    (bool)add_scaleout_device<device_candidate_t,
                                       group_id,
                                       class_id,
                                       context>(storage,
                                                index,
