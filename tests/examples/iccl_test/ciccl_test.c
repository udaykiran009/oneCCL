...
/* ICCL library C binding API usage example and correctness check test */
...
#define ICCL_CALL(expression)                                             \
  do                                                                      \
  {                                                                       \
      int ret = expression;                                               \
      if (ret != CICCL_SUCCESS)                                           \
      {                                                                   \
          printf("%s:%d: ICCL error: ret %d\n", __FILE__, __LINE__, ret); \
          iccl_environment_finalize(env);                                 \
          exit(1);                                                        \
      }                                                                   \
  } while (0)
...
iccl_environment env;
iccl_session session;
iccl_distribution distribution;
...
void init_layer(layer_t* layer, size_t layer_idx, iccl_operation op, layer_t* prev_layer)
{
    layer->op = op;
    layer->layer_idx = layer_idx;
    layer->is_backward_unpack_called = 0;

    ICCL_CALL(iccl_operation_get_distribution(layer->op, &layer->distribution));

    ICCL_CALL(iccl_operation_has_parameter_sets(layer->op, &layer->has_param_set));
    if (layer->has_param_set)
    {
        ICCL_CALL(iccl_operation_get_parameter_set(layer->op, 0 /* idx */, &layer->param_set));
        ICCL_CALL(iccl_parameter_set_get_kernel_size(layer->param_set, &layer->kernel_size));
        ICCL_CALL(iccl_parameter_set_get_local_kernel_count(layer->param_set, &layer->local_kernel_count));
        ICCL_CALL(iccl_parameter_set_get_owned_kernel_count(layer->param_set, &layer->owned_kernel_count));
        ICCL_CALL(iccl_parameter_set_get_owned_kernel_offset(layer->param_set, &layer->owned_kernel_offset));
    }

    ICCL_CALL(iccl_operation_get_input(layer->op, 0 /* idx */, &layer->input_act));
    ICCL_CALL(iccl_activation_get_fm_size(layer->input_act, &layer->input_fm_size));
    ICCL_CALL(iccl_activation_get_local_fm_count(layer->input_act, &layer->input_local_fm_count));
    ICCL_CALL(iccl_activation_get_global_fm_offset(layer->input_act, &layer->input_global_fm_offset));

    ICCL_CALL(iccl_operation_get_output(layer->op, 0 /* idx */, &layer->output_act));
    ICCL_CALL(iccl_activation_get_fm_size(layer->output_act, &layer->output_fm_size));
    ICCL_CALL(iccl_activation_get_local_fm_count(layer->output_act, &layer->output_local_fm_count));
    ICCL_CALL(iccl_activation_get_global_fm_offset(layer->output_act, &layer->output_global_fm_offset));

    ICCL_CALL(iccl_operation_get_local_minibatch_size(layer->op, &layer->local_mb_size));

    ICCL_CALL(iccl_distribution_get_process_count(layer->distribution, GT_DATA, &layer->data_group_size));
    ICCL_CALL(iccl_distribution_get_process_count(layer->distribution, GT_MODEL, &layer->model_group_size));
    ...
    else
    {
        ...
        size_t prev_fm_size;
        ICCL_CALL(iccl_activation_get_local_fm_count(prev_layer->output_act, &prev_local_fm_count));
        ICCL_CALL(iccl_operation_get_local_minibatch_size(prev_layer->op, &prev_local_mb_size));
        ICCL_CALL(iccl_activation_get_fm_size(prev_layer->output_act, &prev_fm_size));
    ...
    if (prev_layer != NULL)
    {
        ...
        ICCL_CALL(iccl_operation_set_prev(layer->op, prev_layer->op, 0, 0));
    }
    ...
    {
        ICCL_CALL(iccl_environment_alloc(env, param_size, CACHELINE_SIZE, (void**)&layer->param_buf));
        ICCL_CALL(iccl_environment_alloc(env, param_size, CACHELINE_SIZE, (void**)&layer->param_grad_buf));
        ICCL_CALL(iccl_environment_alloc(env, param_inc_size, CACHELINE_SIZE, (void**)&layer->param_inc_buf));
    }
...
void finalize_layer(layer_t* layer)
{
    ...
    {
        ICCL_CALL(iccl_environment_free(env, layer->param_buf));
        ICCL_CALL(iccl_environment_free(env, layer->param_grad_buf));
        ICCL_CALL(iccl_environment_free(env, layer->param_inc_buf));
    }
}

void pack_buffer(iccl_activation act, DTYPE* comm_buf, DTYPE* local_buf)
{
    size_t local_fm_count, block_count;
    ICCL_CALL(iccl_activation_get_local_fm_count(act, &local_fm_count));
    ICCL_CALL(iccl_activation_get_pack_block_count(act, &block_count));

    size_t block_idx = 0;
    for (; block_idx < block_count; block_idx++)
    {
        iccl_comm_block_info block;
        ICCL_CALL(iccl_activation_get_pack_block(act, block_idx, &block));

        size_t mb_count, mb_offset, fm_count, fm_offset, fm_size, buf_offset;
        ICCL_CALL(iccl_comm_block_info_get_mb_count(block, &mb_count));
        ICCL_CALL(iccl_comm_block_info_get_mb_offset(block, &mb_offset));
        ICCL_CALL(iccl_comm_block_info_get_fm_count(block, &fm_count));
        ICCL_CALL(iccl_comm_block_info_get_fm_offset(block, &fm_offset));
        ICCL_CALL(iccl_comm_block_info_get_fm_size(block, &fm_size));
        ICCL_CALL(iccl_comm_block_info_get_buf_offset(block, &buf_offset));
...
void unpack_buffer(iccl_activation act, DTYPE* comm_buf, DTYPE* local_buf)
{
    size_t local_fm_count, block_count;
    ICCL_CALL(iccl_activation_get_local_fm_count(act, &local_fm_count));
    ICCL_CALL(iccl_activation_get_unpack_block_count(act, &block_count));

    size_t block_idx = 0;
    for (; block_idx < block_count; block_idx++)
    {
        iccl_comm_block_info block;
        ICCL_CALL(iccl_activation_get_unpack_block(act, block_idx, &block));
...
void forward(layer_t* layer)
{
    ...
    ICCL_CALL(iccl_activation_wait_comm(layer->input_act, (void**)&input_act_comm_buf));
    unpack_buffer(layer->input_act, input_act_comm_buf, layer->input_act_buf);
    if (layer->has_param_set)
    {
        DTYPE* inc_comm_buf;
        ICCL_CALL(iccl_parameter_set_wait_increment_comm(layer->param_set, (void**)&inc_comm_buf));
    }

    forward_compute(layer, layer->input_act_buf, layer->param_buf, layer->output_act_buf);

    DTYPE* output_act_comm_buf;
    ICCL_CALL(iccl_activation_get_comm_buf(layer->output_act, (void**)&output_act_comm_buf));
    pack_buffer(layer->output_act, output_act_comm_buf, layer->output_act_buf);
    ICCL_CALL(iccl_activation_start_comm(layer->output_act, output_act_comm_buf));
    layer->is_backward_unpack_called = 0;
}
...
void backward1(layer_t* layer)
{
    ...
    {
        DTYPE* output_act_comm_buf;
        ICCL_CALL(iccl_activation_wait_comm(layer->output_act, (void**)&output_act_comm_buf));
    ...
    DTYPE* input_act_comm_buf;
    ICCL_CALL(iccl_activation_get_comm_buf(layer->input_act, (void**)&input_act_comm_buf));
    pack_buffer(layer->input_act, input_act_comm_buf, layer->input_act_grad_buf);
    ICCL_CALL(iccl_activation_start_comm(layer->input_act, input_act_comm_buf));
}
...
void backward2(layer_t* layer)
{
    ...
    {
        DTYPE* comm_buf;
        ICCL_CALL(iccl_activation_wait_comm(layer->output_act, (void**)&comm_buf));
    ...
    if (layer->has_param_set)
        ICCL_CALL(iccl_parameter_set_start_gradient_comm(layer->param_set, layer->param_grad_buf));
}
...
void update(layer_t* layer)
{
    ...
    {
        DTYPE* comm_buf;
        ICCL_CALL(iccl_parameter_set_wait_gradient_comm(layer->param_set, (void**)&comm_buf));
        ...
        ICCL_CALL(iccl_parameter_set_start_increment_comm(layer->param_set, layer->param_buf));
    }
}


/* Layer initialization */
layer_t* create_layer(iccl_session ses, iccl_distribution dist, layer_type_t type, layer_params_t* lparams,  layer_t* prev_layer)
{
    MY_ASSERT((type == CONV_MIMO || type == CONV_FLAT || type == FC), "incorrect op type");

    size_t layer_idx = lparams->layer_idx;

    iccl_operation_reg_info reg_info;
    ICCL_CALL(iccl_session_create_operation_reg_info(ses, OT_CC, &reg_info));
    ICCL_CALL(iccl_operation_reg_info_set_name(reg_info, "MyLayerName"));
    ICCL_CALL(iccl_operation_reg_info_add_input(reg_info, lparams->ifm, lparams->ifm_width * lparams->ifm_height, ICCL_DTYPE));
    ICCL_CALL(iccl_operation_reg_info_add_output(reg_info, lparams->ofm, lparams->ofm_width * lparams->ofm_height, ICCL_DTYPE));
    ICCL_CALL(iccl_operation_reg_info_add_parameter_set(reg_info, lparams->ifm * lparams->ofm, lparams->kw * lparams->kh,
                                                        ICCL_DTYPE, use_dist_update));

    size_t op_idx;
    ICCL_CALL(iccl_session_add_operation_with_distribution(ses, reg_info, dist, &op_idx));
    ICCL_CALL(iccl_session_delete_operation_reg_info(ses, reg_info));

    iccl_operation op;
    ICCL_CALL(iccl_session_get_operation(ses, op_idx, &op));
...
int main(int argc, char** argv)
{
    if (argc < 2)
    {
        printf("specify parameters: ciccl_test GROUP_COUNT [DIST_UPDATE] [USER_BUF]\n");
        exit(0);
    }

    int runtime_version;
    ICCL_CALL(iccl_environment_get_version(&runtime_version));
    printf("built with CICCL API version: %d.%d, used CICCL API version: %d.%d\n",
           CICCL_MAJOR_VERSION, CICCL_MINOR_VERSION, CICCL_MAJOR(runtime_version), CICCL_MINOR(runtime_version));

    if (CICCL_MAJOR_VERSION != CICCL_MAJOR(runtime_version))
    {
        printf("incompatible ICCL C API version: %d.%d, exit\n",
               CICCL_MAJOR(runtime_version), CICCL_MINOR(runtime_version));
        return 0;
    }

    ICCL_CALL(iccl_environment_get_env(&env));
    ICCL_CALL(iccl_environment_init(env, &argc, &argv));
    ICCL_CALL(iccl_environment_create_session(env, PT_TRAIN, &session));
    ICCL_CALL(iccl_session_set_global_minibatch_size(session, GLOBAL_MINIBATCH_SIZE));
    ICCL_CALL(iccl_environment_get_process_count(env, &process_count));
    ...
    ICCL_CALL(iccl_environment_get_process_idx(env, &process_idx));
    if (process_idx == 0)
    ...
    ICCL_CALL(iccl_environment_create_distribution(env, process_count/group_count, group_count, &distribution));
    ...
    ICCL_CALL(iccl_session_commit(session));
    ...
    for (epoch_idx = 0; epoch_idx < EPOCH_COUNT; epoch_idx++)
    {
        ...
        for (layer_idx = 0; layer_idx < LAYER_COUNT; layer_idx++)
        {
            ...
            {
                DTYPE* inc_comm_buf;
                ICCL_CALL(iccl_parameter_set_wait_increment_comm(layers[layer_idx]->param_set, (void**)&inc_comm_buf));
            }
        }
    }
    ...
    ICCL_CALL(iccl_environment_delete_session(env, session));
    ICCL_CALL(iccl_environment_delete_distribution(env, distribution));
    ICCL_CALL(iccl_environment_finalize(env));
