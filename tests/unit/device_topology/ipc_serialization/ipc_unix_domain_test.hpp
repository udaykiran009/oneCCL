...
TEST_F(ipc_handles_fixture, ipc_unix_server_test) {
    ...
    for(size_t connection_index = 0; connection_index < connect_iteration; connection_index++)
    {
        ...
        output << "PID: " << *my_pid << ", phase: " << (int)phase << " connect to server: " << other_addr << std::endl;
        std::unique_ptr<ipc_tx_connection> tx_conn;
        try {
            tx_conn.reset(new ipc_tx_connection(other_addr));
        }
        catch (const std::exception& ex) {
            UT_ASSERT(false, "Cannot create TX connection to other IPC server on: " << other_addr
                             << ", error: " << ex.what());
        }
        ...
        output << "PID: " << *my_pid << ", phase: " << (int)phase << " accept conn from my server: " << my_addr << std::endl;
        ...
        try {
            ...
            UT_ASSERT(false, "Cannot accept RX connectio from my IPC server on: " << my_addr
                             << ", error: " << ex.what());
        }
        ...
        output << "PID: " << *my_pid << ", phase: " << (int)phase << " send data: " << tx_send << " to server: " << other_addr << std::endl;
        ...
        try {
            tx_conn->send_msg_with_pid_data(tx_data, {}, 0);
             ...
             UT_ASSERT(false, "Cannot send TX data to other IPC server on: " << other_addr
                             << ", error: " << ex.what());
        }
        ...
        try {
            rx_conn->recv_msg_with_pid_data(rx_data, out_pids_resized, 0);
             ...
             UT_ASSERT(false, "Cannot recv RX data on my IPC server on: " << my_addr
                             << ", error: " << ex.what());
        }
        std::string rx_recv(reinterpret_cast<const char*>(rx_data.data()), rx_data.size());
        output << "PID: " << *my_pid << ", phase: " << (int)phase << " received data: " << rx_recv << " from server: " << my_addr << std::endl;
    ...
    output << "PID: " << *my_pid << ", phase: " << (int)phase << ", servers have stopped" << std::endl;
    ...
    for(size_t connection_index = 0; connection_index < connect_iteration; connection_index++)
    {
        ...
        UT_ASSERT(dropped, "Impossible create TX connection to other IPC server on: " << other_addr);
