...
TEST(event, move_event) {
    ...
    ccl::event orig_ev3;
    ccl::event moved_ev3(std::move(orig_ev3.get_impl()));
...
TEST(event, native_event) {
        ...
        ccl::event::native_t native_ev; // this is shared_ptr for all, exclude sycl
        ccl::event new_ev = ccl::event::create_from_native(native_ev);
        ccl::event::native_t new_native_ev = new_ev.get_native();
        (void)new_native_ev;
    #endif
}
