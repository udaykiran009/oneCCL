...
namespace stub
{
    struct test_device : public native::ccl_device
    {
        test_device(native::ccl_device::owner_ptr_t&& parent) :
            native::ccl_device(reinterpret_cast<native::ccl_device::handle_t>(new native::ccl_device::handle_t), std::move(parent), std::false_type{})
        {
        }
                                                   ...
                                                   native::ccl_device::owner_ptr_t&& driver)
        {
            ...
            using top_class_integer_type = typename std::underlying_type<ccl::device_topology_type>::type;
    ...
    struct test_subdevice : public native::ccl_subdevice
    {
        test_subdevice(native::ccl_subdevice::owner_ptr_t&& parent,
            ...
            native::ccl_subdevice(reinterpret_cast<native::ccl_subdevice::handle_t>(new native::ccl_subdevice::handle_t), std::move(parent), std::move(driver), std::false_type{})
        {
        }

        static std::shared_ptr<test_subdevice> create(const ccl::device_index_type& full_device_index,
                                                   native::ccl_subdevice::owner_ptr_t&& device,
                                                   typename native::ccl_subdevice::base::owner_ptr_t&& driver)
        {
            std::shared_ptr<test_subdevice> subdev =
                        std::make_shared<test_subdevice>(std::move(device),
                                                         std::move(driver));
            subdev->device_properties.type = ZE_DEVICE_TYPE_GPU;
            subdev->device_properties.version =
                        ZE_DEVICE_PROPERTIES_VERSION_CURRENT;
            ...
            queue_prop.ordinal = 0;
            subdev->cmd_queus.emplace(queue_prop, ccl_subdevice::device_queue{nullptr, subdev->get_ptr()});

            //create module
            auto module_ptr = std::make_shared<ccl_subdevice::device_module>(nullptr, subdev->get_ptr());
    ...
    inline void make_stub_devices(const ccl::device_indices_t& stub_indices)
    {
        ...
        for(const auto& index : stub_indices)
        {
            ...
            {
                driver_it = drivers.emplace(driver_index,
                                            ccl_device_driver::create(nullptr,
                                                                     driver_index,
                                                                     platform.get_ptr())).first;
            }
            ...
            if (device_it == devices.end())
            {
                ...
                device_it = devices.emplace(device_index,
                                            test_device::create(dev_idx,
                                                                driver.get_ptr())).first;
            }
            ...
            {
                subdevice_it = subdevices.emplace(subdevice_index,
                                                  test_subdevice::create(index,
                                                                         device.get_ptr(),
                                                                         driver.get_ptr())).first;
            }
        }
