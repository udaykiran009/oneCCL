...
TEST_F(ipc_one_dev_fixture, ring_allreduce_one_device)
{
    ...
    for(size_t thread_idx = 0; thread_idx < num_thread; thread_idx++)
    {
        ...
        try
        {
            ...
            auto temp_recv = device.alloc_memory<native_type>(buffer_size / num_thread, sizeof(native_type));
            mem_send.enqueue_write_sync(send_values);
            mem_recv.enqueue_write_sync(recv_values);
            temp_recv.enqueue_write_sync(recv_values.begin(), recv_values.begin() + buffer_size / num_thread);
             ...
             */
            memory_storage.register_shared_data(thread_idx, num_thread,
                                                std::move(mem_send),
                                                std::move(mem_recv),
                                                std::move(temp_recv));
            ...
            ipc_server_flags.create_ipcs(thread_idx, num_thread, &left_wrote_2_me_flag, &read_for_receive_flag);

            /* fill array in specific order
             * Left: l_L, l_R, l_B, r_L, r_R
             * Right: r_L, r_R, r_B, l_L, L_R
             */
            flags_storage.register_shared_data(thread_idx, num_thread,
        ...
        {
            UT_ASSERT(false, "Cannot allocate memory for thread: %" << thread_idx
                             << "\nError: " << ex.what());
        }
    }
    ...
    for(size_t thread_id = 0; thread_id < num_thread; thread_id++)
    {
        ...
        serialized_raw_handles.insert(serialized_raw_handles.end(), memory_serialized[thread_id].begin(), memory_serialized[thread_id].end());

        flags_serialized[thread_id] = ipc_server_flags.serialize_storage(thread_id);
        serialized_raw_handles.insert(serialized_raw_handles.end(), flags_serialized[thread_id].begin(), flags_serialized[thread_id].end());
    }


    // send to other process
    output << "PID: " << pid <<  " write to socket bytes: " << serialized_raw_handles.size() << std::endl;
    ...
    output << "PID: " << pid <<  " read from socket,expected bytes: " << received_raw_handles.size() << std::endl;
    ...
    try
    {
        ...
        for(size_t thread_id = 0; thread_id < num_thread; thread_id++)
        {
            ...
            memory_serialized[thread_id].assign(received_raw_handles.begin(), received_raw_handles.begin() + mem_send);
            flags_serialized[thread_id].assign(received_raw_handles.begin() + mem_send, received_raw_handles.end());

            size_t count = ipc_client_memory.deserialize(memory_serialized.at(thread_id), 1, *global_platform);
            UT_ASSERT( count == 1, "Deserialized 1 IPC memory handle");
            count = ipc_client_flags.deserialize(flags_serialized.at(thread_id), 2, *global_platform);
    ...
    for(auto &idx_kernel : thread_kernels)
    {
        ...
        thread_group.emplace_back([this, &driver, &device,
                                   thread_idx, kernel, &list, &queue,
                                   &mem_handles, &flag_handles,
                                   ipc_mem_handles, ipc_flag_handles,
                                   &comm_handles,
                                   &thread_lock, &val, raw_out]()
        {
            (void)driver;
            (void)device;
            (void)flag_handles;
            ...
            try
            {
                ...
                for(auto& comm : comm_handles)
                {
                    ...
                    if (result != ZE_RESULT_SUCCESS)
                    {
                        throw std::runtime_error(std::string("Cannot zeKernelSetArgumentValue memory at comm_offset: ") +
                                                std::to_string(comm_offset[i]) + " index\nError: " +
                ...
                for(auto &mem : mem_handles)
                {
                    ...
                    if (result != ZE_RESULT_SUCCESS)
                    {
                        throw std::runtime_error(std::string("Cannot zeKernelSetArgumentValue memory at mem_offset: ") +
                ...
                try
                {
                    ...
                    out << "PID: " << pid << ", thread_idx: " << thread_idx << ", ipc_mem_handles: \n";
                    for(auto &mem : ipc_mem_handles)
                    {
                        ...
                        result = zeKernelSetArgumentValue(kernel, arg_offset, sizeof(mem->handle), &mem->handle);
                        if (result != ZE_RESULT_SUCCESS)
                        {
                            throw std::runtime_error(std::string("Cannot zeKernelSetArgumentValue memory at mem_offset: ") +
                                                    std::to_string(arg_offset) + " index\nError: " +
                ...
                {
                    out<< "PID: " << pid << ", Cannot bind IPC memory, error: " << ex.what() << std::endl;
                    throw;
                }
                ...
                for(auto &flag: flag_handles)
                {
                    ...
                    if (result != ZE_RESULT_SUCCESS)
                    {
                        throw std::runtime_error(std::string("Cannot zeKernelSetArgumentValue flags at flag_offset: ") +
                ...
                try
                {
                    ...
                    out<< "PID: " << pid << ", thread_idx: " << thread_idx << ", ipc_flag_handles: \n" << std::endl;
                    for(auto &flag : ipc_flag_handles)
                    {
                        int arg_offset = mem_offset.at(i);
                        out << "index: " << arg_offset << ": " << flag->handle.pointer << std::endl;
                        result = zeKernelSetArgumentValue(kernel, arg_offset, sizeof(flag->handle), &flag->handle);
                        if (result != ZE_RESULT_SUCCESS)
                        {
                            throw std::runtime_error(std::string("Cannot zeKernelSetArgumentValue flag at mem_offset: ") +
                                                    std::to_string(arg_offset) + " index\nError: " +
                                                    native::to_string(result));
                        }
                        i++;
                ...
                {
                    out<< "PID: " << pid << ", Cannot bind IPC flag, error: " << ex.what() << std::endl;
                    throw;
                }
                ...
                if (thread_idx == 0/* && ret_data == 1*//*uncommet for master pid*/)
                {
                    ...
                    if(ret != ZE_RESULT_SUCCESS )
                    {
                        throw std::runtime_error(std::string("cannot zeCommandListClose, error: ") + std::to_string(ret));
                    }
                    ...
                    if(ret != ZE_RESULT_SUCCESS )
                    {
                        throw std::runtime_error(std::string("cannot zeCommandQueueExecuteCommandLists, error: ") + std::to_string(ret));
                    }
                    ...
                    do
                    {
                        ...
                        {
                            throw std::runtime_error(std::string("cannot zeCommandQueueSynchronize, error: ") + std::to_string(ret));
                        }
            ...
            {
                UT_ASSERT(false, "Exception in PID: " << pid << ",thread: " << thread_idx
