// test case data
namespace multi_tile_single_device_case
{
...
TEST_F(allreduce_one_device_multi_tile_local_fixture, allreduce_multi_tile_one_device_kernel)
{
    ...
    UT_ASSERT(subdevices.size() == num_thread, "Subdevices count: " <<  subdevices.size()
    ...
    for(auto dev_it = subdevices.begin(); dev_it != subdevices.end(); ++dev_it)
    {
         ...
         */
        memory_storage.register_shared_data(rank_device_idx, num_thread,
                                            std::move(mem_send),
                                            std::move(mem_recv),
                                            std::move(temp_recv));
         ...
         */
        flags_storage.register_shared_data(rank_device_idx, num_thread,
    ...
    for(auto dev_it = subdevices.begin(); dev_it != subdevices.end(); ++dev_it)
    {
        ...
        thread_group.emplace_back([this, driver, &subdevice,
