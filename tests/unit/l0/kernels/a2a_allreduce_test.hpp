...
TEST_F(allreduce_one_device_local_fixture, a2a_allreduce_one_device_multithread_kernel)
{
    ...
    //get handles for A2A
    typename handles_storage<native_type>::thread_handles_container rank_mem   = memory_storage.collect_handles_by_index({2});
    typename handles_storage<int>::thread_handles_container rank_flags         = flags_storage.collect_handles_by_index({0,1});
    ...
    //prepare gpu object
    auto a2a_comm_handle = device.alloc_memory<a2a_gpu_comm_data_float>(num_thread, sizeof(a2a_gpu_comm_data_float));
    ...
    for(auto &idx_kernel : thread_kernels)
    {
        ...
        thread_group.emplace_back([this, &a2a_comm_handle,
                                   ...
                                   &thread_lock, &val, raw_out]()
        {
            ...
            try
            {
...

                {
                    ...
                    out << "index: " << a2a_comm_offset << ": " << a2a_comm_handle.handle << std::endl;
                    result = zeKernelSetArgumentValue(kernel, a2a_comm_offset, sizeof(a2a_comm_handle.handle), &a2a_comm_handle.handle);
                    if (result != ZE_RESULT_SUCCESS)
                    {
                        throw std::runtime_error(std::string("Cannot zeKernelSetArgumentValue a2a comm at a2a_comm_offset: ") +
                                                std::to_string(a2a_comm_offset) + " index\nError: " +
                                                native::to_string(result));
                    }
                }
                out << std::endl;
