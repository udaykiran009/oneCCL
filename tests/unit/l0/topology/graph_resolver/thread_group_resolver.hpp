...
TEST_F(router_fixture, thread_graph_resolver_test_devices_2_not_p2p) {
    ...
    ccl::process_device_indices_type indices{
        { 0,
          {
    ...
    std::stringstream ss;
    adjacency_matrix matrix = thread_group_ring_topology::build_p2p_capability_matrix(
        ss,
        indices,
        std::bind(
    ...
    plain_graph_list graphs = graph_list_resolver(
        matrix,
        indices,
        std::bind(
            test_custom_p2p_ping, std::placeholders::_1, std::placeholders::_2, expected_matrix));
    std::tie(res, descr) =
        check_multiple_graph_ring(graphs,
                                  { {
                                        ccl::device_index_type(0, 0, ccl::unused_index_value),
                                    },
                                    { ccl::device_index_type(0, 1, ccl::unused_index_value) } });
