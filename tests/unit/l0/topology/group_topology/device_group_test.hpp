...
TEST_F(router_fixture, real_virtual_topology_test)
{
    ...
    constexpr ccl::device_topology_type class_id = ccl::device_topology_type::ring;
    {
        ...
        try
        {
            res = top.build(ss, tg_comm->thread_device_group_ctx[thread_index]->context_addr, get_device_affinity(thread_index), matrix);
        }
        ...
        std::tie(res, descr) =
                check_topology<topology>(
                    tg_comm->get_device_group_ctx(thread_index)->get_group_topology<class_id>().get_topology(0)->devices,
...
TEST_F(router_fixture, multiple_real_topology_test)
{
...

    {
        ...
        //Check topology
        std::tie(res, descr) = check_topology<topology>(tg_comm->get_device_group_ctx(thread_index)->get_group_topology<class_id>().get_topology(0)->devices,
...
TEST_F(router_fixture, two_inaccessible_group_scaleup_test)
{
    ...
    constexpr ccl::device_topology_type class_id = ccl::device_topology_type::ring;
    {
                                       ...
                                       *pg_comm->gpu_device_storage);
        {
            ...
            //Check topology
            std::tie(res, descr) = check_topology<topology>(group_ctx->get_group_topology<class_id>().get_additiona_topology(0)->devices,
                                                            expected_indices_tuple(
                                                                {
