                                        ...
                                        native::detail::p2p_rating_function ping) override
{
    ...
    if (!overrided_cluster_graphs_to_replace.empty())
    {
        ...
        return allied_process_group_ring_topology::merge_allied_nodes_plain_graphs(out, cluster_indices,
                                                                               process_index,
                                                                               overrided_cluster_graphs_to_replace,
                                                                               ping);
    }
                                        ...
                                        std::ostream& out) override
{
    ...
    if (!overrided_colored_merged_cluster_graphs_to_check.empty())
    {
        for (const auto& initial : cluster_graphs)
        {
            auto tested_it = overrided_colored_merged_cluster_graphs_to_check.find(initial.first);
    ...
    return allied_process_group_ring_topology::create_scaleout_devices_in_colored_graphs_for_process(
                                                                            process_index,
                                                                            mock_cluster_size,
                                                                            cluster_graphs,
                                                                            initial_cluster_graphs,
                                                                            out);
}
        ...
        std::initializer_list<typename native::detail::global_sorted_colored_plain_graphs::value_type> to_replace = {})
{
    ...
    {
        overrided_colored_merged_cluster_graphs_to_replace = overrided_colored_merged_cluster_graphs_to_check;
    }
}
