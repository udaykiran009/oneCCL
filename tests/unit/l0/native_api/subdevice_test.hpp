...
TEST_F(allreduce_one_device_local_fixture, subdevice_2_tiles_test)
{
...

    {
        ...
        UT_ASSERT(one_device->get_device_properties().isSubdevice == false, "Should not be subdevice. Properties:\n"
                  << one_device->to_string());

        auto device_id = one_device->get_device_id();
        UT_ASSERT(one_device->get_device_properties().deviceId == device_id, "DeviceId is failed. Properties:\n"
                  << one_device->to_string());
        for(auto &subdev : subdevices)
        {
            UT_ASSERT(subdev.second->get_device_properties().isSubdevice == true, "Must be subdevice. Properties:\n"
                  << subdev.second->to_string());
            auto subdevice_id = one_device->get_device_id();
            UT_ASSERT(one_device->get_device_properties().subdeviceId == subdevice_id, "Sub DeviceId is failed. Properties:\n"
                  << one_device->to_string());
        }
    }
    {
    ...
    }
    {
        ...
        UT_ASSERT(device_mem.get_owner().lock().get() == one_device.get(), "Device memory should have one_device parent");
        for(auto &subdev : subdevices)
        {
            ...
            UT_ASSERT(device_mem.get_owner().lock().get() != subdevice_mem.get_owner().lock().get(), "Mem owners should be different");
            UT_ASSERT(subdevice_mem.get_owner().lock().get() == subdev.second.get(), "Sub device mem do not belongto subdevice ");
            std::shared_ptr<ccl_subdevice> subdevice = std::dynamic_pointer_cast<ccl_subdevice>(subdevice_mem.get_owner().lock());
            UT_ASSERT(subdevice->parent_device.lock().get() == one_device.get(), "Sub device mem has not parent of one_device parent");
        }
    }
}
}
