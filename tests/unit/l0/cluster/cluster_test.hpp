    ...
    cluster_mask.insert({case_name,
                                  {
                                      ...
                                      {1, ccl::device_mask_t("1100")}
                                  }
                        });
    allied_processes_mask = cluster_mask[case_name];
    std::tie(check, err_descr) = std::make_tuple<bool, std::string>(true, "TODO"); //process_group_context::check_device_mask_validity_across_allied_processes(allied_processes_mask);
...
TEST_F(communicator_fixture, build_allied_processes_affinity_mask)
{
    ...
    auto ccl_comm = get_fixture_comm();
    {
        ...
        cluster_mask.insert(
                            {case_name, ccl::process_device_indices_t
                                                {
                                                    {proces_idx, process_mask.find(proces_idx)->second}
                                                }
                            });

        output << "initialize global mask for host: " << p_group_comm.get_host_id() << ", process idx: " << proces_idx << std::endl;
        ...
        {
            UT_ASSERT(val.first < process_mask.size(), "Invalid process id: " << val.first
...

    {
        ...
        const auto& node_mask = p_group_comm.get_node_afinity_indices(p_group_comm.get_host_id());
        UT_ASSERT(node_mask.size() == 1, "Invalid nodes count in affinity : " << node_mask.size()
                                                           << ", expected: " << 1);
