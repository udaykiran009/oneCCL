...
bool observer_event<TEMPLATE_DEF_ARG>::consume_data()
{
    ...
    {
        //fence
        LOG_TRACE(class_name(), ": ", static_cast<void*>(this), " - bytes produced: ", total_produced,
                  ", previously bytes consumed: ", old_consumed);
        std::atomic_thread_fence(std::memory_order::memory_order_seq_cst);

        //read data
        element_t* produced_chunk_ptr = ready_memory_chunk->get() + old_consumed;
        std::copy(produced_chunk_ptr, produced_chunk_ptr + total_produced - old_consumed,
    ...
    {
        CCL_ASSERT(bytes_consumed == bytes_expected, "Unexpected consumed: ", bytes_consumed,
