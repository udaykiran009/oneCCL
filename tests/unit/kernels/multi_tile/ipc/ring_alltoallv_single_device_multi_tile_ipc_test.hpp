...
TYPED_TEST(ring_alltoallv_multi_process_fixture, ring_alltoallv_single_device_multi_tile_ipc) {
    ...
    for (auto dev_it = devices.begin(); dev_it != devices.end(); ++dev_it) {
        try {
            ...
            mem_send_counts.enqueue_write_sync(send_counts[rank_idx]);
            mem_send_offsets.enqueue_write_sync(send_offsets[rank_idx]);

            comm_param_mem_storage[rank_idx].emplace_back(std::move(mem_send_counts));
            comm_param_mem_storage[rank_idx].emplace_back(std::move(mem_send_offsets));
            ...
            mem_recv_counts.enqueue_write_sync(recv_counts[rank_idx]);
            mem_recv_offsets.enqueue_write_sync(recv_offsets[rank_idx]);
            ...
            auto mem_send = sub_device->alloc_memory<native_type>(
                total_send_sizes[rank_idx], sizeof(native_type), ctx);
            auto mem_recv =
            ...
            mem_send.enqueue_write_sync(send_values[rank_idx]);
            mem_recv.enqueue_write_sync(recv_values[rank_idx]);
            mem_tmp.enqueue_write_sync(tmp_values);

            this->template register_ipc_memories_data<native_type>(ctx, rank_device_idx, &mem_tmp);
    ...
    std::stringstream ss;
    bool ret = alltoallv_checking_results<native_type>(this, world_size, ss);
