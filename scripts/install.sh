...
if [ "$USERNAME" = "root" ]
then
    DEFAULT_INSTALL_PATH="/opt/intel/CCL_SUBSTITUTE_OFFICIAL_VERSION"
else
    DEFAULT_INSTALL_PATH="${HOME}/intel/CCL_SUBSTITUTE_OFFICIAL_VERSION"
fi
...
if [ ${SILENT_MODE} -eq 0 ]
then
    echo "oneCCL 2021.5.0 for Linux* OS will be installed"
...
if [ ${SILENT_MODE} -eq 0 ]
then
    echo "oneCCL 2021.5.0 for Linux* OS will be installed into ${INSTALL_PATH}"
fi
