...
print_help()
{
    echo "Usage: $ME --name NAME --old PATH --new PATH --report PATH"
    ...
    echo "     $ME --name libccl_cpu --old ./mlsl_old/build --new ./mlsl_new/build --report-dir ./report"
    echo "     or"
    echo "     $ME --name libccl_gpu --old ./mlsl_old/build_gpu --new ./mlsl_new/build_gpu --report-dir ./report"
    echo
    echo "  2) Comparison of two builds after the post build step:"
    echo "     $ME --name libccl_cpu --old ./mlsl_old/build --new ./mlsl_new/build --report-dir ./report --build-type cpu_icc"
    echo "     or"
    echo "     $ME --name libccl_gpu --old ./mlsl_old/build --new ./mlsl_new/build --report-dir ./report --build-type cpu_gpu_dpcpp"
}
...
set_env()
{
    ...
    UTILS_DIR=/nfs/inn/disks/nn-ssg_tcar_mpi_2Tb_unix/jenkins/artefacts/jenkins-utils/CCL/abi
