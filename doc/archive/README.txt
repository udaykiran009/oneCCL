-----------------------------------
Short README for MLSL build and test
-----------------------------------
...
2. Build:
   ...
   This builds libmlsl.a library and mlsl_test test application in bin directory.

3. Run test app
   Simply run mlsl_test using mpiexec.hydra with desired number of nodes (N). It takes single parameter "num_groups"
   ...
   NUM_NODES=8
   # Data parallelism
   $mpiexec.hydra -np ${NUM_NODES} bin/mlsl_test 1

   # Model parallelism
   $mpiexec.hydra -np ${NUM_NODES} bin/mlsl_test ${NUM_NODES}

   # Hybrid parallelism (NUM_NODES/2) x 2
   $mpiexec.hydra -np ${NUM_NODES} bin/mlsl_test 2
...
4. Test setup and Sample Output
   ...
   mpiexec.hydra  -np 8 ./bin/mlsl_test 2 > tests/sample_output.txt
