...
private:
    ...
    void set_common_fields(std::integral_constant<ccl::operation_attr_id, ccl::operation_attr_id::match_id>, const std::string& match)
    {
        ...
        {
            throw ccl::ccl_error(std::string("Too long `match_id` size: ") + std::to_string(match.size()) +
...
struct cpu_specific_data
{
    ...
    static void init(size_t size, size_t rank, ccl::shared_ptr_class<ccl::kvs_interface> kvs)
    {
        ...
        comm_ptr = std::make_shared<ccl::communicator>(ccl::environment::instance().create_communicator(size, rank, kvs));
    }
...
struct device_specific_data
{
    ...
    static void init(size_t size, size_t rank, cl::sycl::device &device, cl::sycl::context ctx, ccl::shared_ptr_class<ccl::kvs_interface> kvs)
    {
        ...
        comm_array = ccl::environment::instance().create_device_communicators(size,
                                                                ccl::vector_class<ccl::pair_class<ccl::rank_t, cl::sycl::device>>{{rank, device}},
                                                                ctx, kvs);
        //single device version
        comm_ptr = device_communicator_ptr( new ccl::device_communicator(
                            std::move(*comm_array.begin())));
        stream_ptr = std::make_shared<ccl::stream>(ccl::environment::instance().create_stream(sycl_queue));
    }
