...
struct cpu_sparse_allreduce_coll :
        base_sparse_allreduce_coll<VType*, IType*, IndicesDistributorType>, cpu_specific_data
{
    ...
    cpu_sparse_allreduce_coll(bench_coll_init_attr init_attr,
                              ...
                              size_t rbuf_size_modifier = 1) : coll_base(init_attr, comm().size())
    {
        int result = 0;
        ...
        single_fn_ctx.recv_ibuf_count = single_buf_max_elem_count * rbuf_size_modifier * comm().size();
        single_fn_ctx.recv_vbuf_count = single_buf_max_elem_count * rbuf_size_modifier * comm().size();
    }
