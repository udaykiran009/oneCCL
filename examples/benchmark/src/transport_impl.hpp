...
void transport_data::init_comms(user_options_t& options) {
    ...
    std::vector<int> local_ranks_vec;
    for (int idx = 0; idx < ranks_per_proc; idx++) {
        local_ranks_vec.push_back(rank * ranks_per_proc + idx);
    }
    ...
    if (options.backend == BACKEND_HOST) {
        ...
        auto sycl_queues = create_sycl_queues(
            sycl_dev_names[options.sycl_dev_type], local_ranks_vec, options.sycl_root_dev);
    ...
    for (int idx = 0; idx < ranks_per_proc; idx++) {
        r2d_map.emplace(local_ranks_vec[idx], devices[idx]);
    }
