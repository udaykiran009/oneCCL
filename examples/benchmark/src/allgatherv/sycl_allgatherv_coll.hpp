    ...
    virtual void finalize_internal(size_t elem_count,
        ...
        for (size_t b_idx = 0; b_idx < base_coll::get_buf_count(); b_idx++) {
            ...
            if (base_coll::get_sycl_mem_type() == SYCL_MEM_USM) {
                ...
                stream.get_native().memcpy(host_send_buf.data(),
                                           send_bufs[b_idx][rank_idx],
                                           send_bytes).wait();

                stream.get_native().memcpy(host_recv_buf.data(),
                                           recv_bufs[b_idx][rank_idx],
                                           recv_bytes).wait();
            }
            else {
                ...
                stream.get_native().memcpy(host_send_buf.data(),
                                           send_buf_acc.get_pointer(),
                                           send_bytes).wait();

                stream.get_native().memcpy(host_recv_buf.data(),
                                           recv_buf_acc.get_pointer(),
                                           recv_bytes).wait();
            }
