...
void run_test(size_t thread_idx,
              const size_t size,
...
int main(int argc, char** argv)
{
    ...
    std::vector<std::vector<size_t>> device_ids(thread_count);
    for(int thread_id = 0; thread_id < thread_count; thread_id++)
    {
        device_ids[thread_id].resize(devices_count);
    }
    std::vector<std::thread> thread_group;

    for(int thread_id = 0; thread_id < thread_count; thread_id++)
    {
        thread_group.emplace_back(&run_test, thread_id, size * thread_count * devices_count, device_ids[thread_id], kvs);
    }
