...
iccl_status_t do_prologue_float_2x(const void* in_buf, size_t in_count, iccl_datatype_t in_dtype,
                                   void** out_buf, size_t* out_count, iccl_datatype_t* out_dtype,
                                   size_t* out_dtype_size)
{
    size_t idx;
    ASSERT(in_dtype == iccl_dtype_float, "unexpected dtype %d", in_dtype);
    ASSERT(out_buf, "null ptr");

    if (out_buf) *out_buf = (void*)in_buf;
    if (out_count) *out_count = in_count;
    if (out_dtype) *out_dtype = iccl_dtype_float;
...
iccl_status_t do_epilogue_float_2x(const void* in_buf, size_t in_count, iccl_datatype_t in_dtype,
                                   void* out_buf, size_t* out_count, iccl_datatype_t out_dtype)
{
    size_t idx;
    ASSERT(in_dtype == iccl_dtype_float, "unexpected dtype %d", in_dtype);
    ASSERT(out_dtype == iccl_dtype_float, "unexpected dtype %d", out_dtype);
    if (out_count) *out_count = in_count;
    for (idx = 0; idx < in_count; idx++)
    {
        ((float*)out_buf)[idx] = ((float*)in_buf)[idx] * 2;
...
iccl_status_t do_prologue_float_to_char(const void* in_buf, size_t in_count, iccl_datatype_t in_dtype,
                                        void** out_buf, size_t* out_count, iccl_datatype_t* out_dtype,
                                        size_t* out_dtype_size)
{
    size_t idx;
...
iccl_status_t do_epilogue_char_to_float(const void* in_buf, size_t in_count, iccl_datatype_t in_dtype,
                                        void* out_buf, size_t* out_count, iccl_datatype_t out_dtype)
{
    size_t idx;
    ASSERT(in_dtype == iccl_dtype_char, "unexpected dtype %d", in_dtype);
    ASSERT(out_dtype == iccl_dtype_float, "unexpected dtype %d", out_dtype);
    if (out_count) *out_count = in_count;
    for (idx = 0; idx < in_count; idx++)
    {
...
iccl_status_t do_reduction_sum(const void* in_buf, size_t in_count, void* inout_buf,
...
int main()
{
    ...
    RUN_COLLECTIVE(iccl_allreduce(send_buf, recv_buf, COUNT, iccl_dtype_float, iccl_reduction_custom, &coll_attr, NULL, &request),
