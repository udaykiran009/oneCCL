...
int main(int argc, char** argv)
{
    ...
    utils::str_to_array<std::string>(affinity_env_value,
    ...
    if (mpi_size != process_group_gpu_affinity.size())
    {
        std::cerr << "L0_CLUSTER_AFFINITY_MASK is configured for processes: " << process_group_gpu_affinity.size()
                  << ", but MPI requested world size: " << mpi_size << ". Both should to be equal" << std::endl;
        return -1;
    }
    ...
    for(size_t process_index = 0; process_index < process_group_gpu_affinity.size(); process_index++)
    {
        ...
        for(size_t thread_index = 0; thread_index < thread_gpu_affinity.size(); thread_index++)
        {
            ...
            utils::str_to_mset<ccl::device_index_type>(thread_gpu_affinity[thread_index].c_str(), device_group_affinity, ',');

            std::cout << " Extracted GPU indices for thread by id: " << thread_index << ", devices in threads count: " << device_group_affinity.size() << std::endl;
            ...
            if(process_index == mpi_rank)
            {
                ...
                {
                    sycl_devices_in_mpi_rank[thread_index].push_back(ccl::create_from_index(device_vendor_id).device);
                }
            }
#endif
        }
    ...
    std::cout << "Devices in cluster count: " << total_device_in_cluster << ", for rank: " << mpi_rank
...
#else
template<class processing_type>
void user_thread_idx(size_t thread_idx, ccl::device_indices_t thread_device_idx,
                     ...
                     std::shared_ptr<ccl::kvs_interface> kvs)
{
    ...
    std::vector<ccl::device_communicator> comms =
                ccl::environment::instance().create_device_communicators(total_devices_in_cluster,
                                                                         thread_device_idx, ctx, kvs);

    std::cout << "Create device communicators, expected count: " << thread_device_idx.size() << std::endl;
